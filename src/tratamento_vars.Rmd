...
#criar uma função que pega o dataframe de coeficientes do summary
#do modelo e retorne o mesmo dataframe, só que somente com as
#variáveis significativas do modelo e o resto das variáveis
#agrupadas de acordo com um intervalo próximo

#parâmetros:
# - dataframe com os coeficientes
# - vetor com as variáveis categóricas


filterSignificantVars = function(data_coef_model, vec_cat_vars, signif_nivel = 0.05){
  
  lapply(vec_cat_vars, function(cat_var){
      data_coef_model %>% 
        filter(str_detect(`Covars Names`,cat_var)) %>% 
        filter(`Pr(>|t|)` < signif_nivel) %>% 
...
filterSignificantVars(coefs_model, signif_cat_vars)
